...
export default function SignIn({ corporate = false }: { corporate?: boolean }) {
  ...
  const onSubmit = async (values: SignInValues) => {
    ...
    try {
      ...
      navigate(corporate ? '/corporate/dashboard' : from && from !== '/signin' ? from : '/dashboard', { replace: true })
    } catch (error) {
  ...
  return (
    <AuthLayout
      ...
      topAction={
        <Link to={corporate ? '/signin' : '/corporate/signin'} className="text-sm font-semibold text-flow-600 hover:underline">
          {corporate ? 'Employee sign in' : 'Corporate sign in'}
        </Link>
      }
      footer={
        <p className="text-sm text-away">
          {corporate ? (
            <>Employee access? <Link to="/signin" className="font-semibold text-flow-600 hover:underline">Sign in here</Link></>
          ) : (
            <>No account yet? <Link to="/signup" className="font-semibold text-flow-600 hover:underline">Request employee access</Link></>
          )}
        </p>
      }
    >
